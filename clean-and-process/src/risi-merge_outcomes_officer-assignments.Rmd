...
stops <-
    stops %>%
    mutate(risi_offduty =
               if_else(stop_officer_id %in% stops_shift_assignments$stop_officer_id,
                       0,
                       1))
...
arrests <-
    arrests %>%
    mutate(risi_offduty =
               if_else(arrest_officer_id %in% arrests_shift_assignments$arrest_officer_id,
                       0,
                       1))
...
force <-
    force %>%
    mutate(risi_offduty =
               if_else(force_id %in% force_shift_assignments$force_id, 0, 1))
```
