...
kable(head(nr_times_working_shift, 50))
...
* These graphs are visual answers to the above question of whether or not officers who work together are more likely to make stops/arrests together.
...
ggplot(stops_together, aes(x = any_off_duty)) +
    geom_bar() +
    theme_bw() +
    labs(x = "Did the stop involve at least one off-duty officer?")

ggplot(stops_together, aes(x = all_off_duty)) +
    geom_bar() +
    theme_bw() +
    labs(x = "Did the stop involve only off-duty officer(s)?")
...
a <-
    shift_ids %>%
    count(unit, beat_assigned, shift, officer_race) %>%
    group_by(unit, beat_assigned, officer_race) %>%
    mutate(n2 = sum(n)) %>% arrange(unit, beat_assigned, officer_race) %>%
    ungroup() %>%
    mutate(prcnt = n / n2) %>%
    group_by(unit, beat_assigned) %>%
    filter(length(unique(officer_race)) > 1) %>%
    group_by(unit, beat_assigned, officer_race) %>%
    mutate(shift_max = if_else(prcnt == max(prcnt), shift, F)) %>%
    ungroup()
```
