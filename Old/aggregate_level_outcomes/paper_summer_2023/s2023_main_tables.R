...
unit_level_descriptive_vars <-
    unit_level %>%
    mutate(prcnt_civ_black = prcnt_civ_black * 100) %>%
...
unit_level_model_vars <-
    ...
    select(
        ...
        mean_years_worked_unit, nr_officer, white_stops, hispanic_stops, white,
        hispanic, total_pop
    )
...
make_df <-
    function(x, name, model) {
        tibble(
            coefficient = x, fixed_effect_category = name, model = model,
            fixed_effect = names(x)
        )
    }
...
model1_fixef <- fixef(nb_ratio)
model2_fixef <- fixef(nb_prcnt)
model3_fixef <- fixef(nb_ratio_exp)
model4_fixef <- fixef(nb_prcnt_exp)

model1_fixef_df <-
    pmap(
        list(model1_fixef, as.list(names(model1_fixef))),
        make_df,
        model = "Model 1"
    ) %>%
    bind_rows()
...
model2_fixef_df <-
    pmap(
        list(model2_fixef, as.list(names(model2_fixef))),
        make_df,
        model = "Model 2"
    ) %>%
    bind_rows()
...
model3_fixef_df <-
    pmap(
        list(model3_fixef, as.list(names(model3_fixef))),
        make_df,
        model = "Model 3"
    ) %>%
    bind_rows()
...
model4_fixef_df <-
    pmap(
        list(model4_fixef, as.list(names(model4_fixef))),
        make_df,
        model = "Model 4"
    ) %>%
    bind_rows()

all_models <- bind_rows(model1_fixef_df, model2_fixef_df, model3_fixef_df, model4_fixef_df)
flextable::save_as_docx(flextable::flextable(all_models), path = file.path(dir, "fixed_effects.docx"))
...
modelsummary(
    list("Stops of All Civilians" = nb_ratio_exp_all,
         "Stops of Black Civilians" = nb_ratio_exp,
    ...
    gof_omit = "R2|RMSE",
    exponentiate = T,
    output = file.path(dir, "table2_stops-aggregate-race.txt"),
    add_rows = offset_row,
    notes = c("Standard Errors in parentheses. Coefficients are incident rate ratios.",
              "P-values are denoted by symbols: + p < 0.1, * p < 0.05, ** p < 0.01, *** p < 0.001"
    )
