...
corr <-
    unit_level_types_stop_race %>%
    group_by(unit) %>%
    summarise(cor(black_ratio, prcnt_officer_black))
...
stop_type_race <-
    read_csv(file.path(dir, "stop_type_race.csv")) %>%
    select(matches("black|shift_id"))

types_of_stops_race <-
    shift_assignments %>%
    full_join(stop_type_race) %>%
    ...
    summarise(
        nr_stops_suspicious = sum(stops_suspicious_black, na.rm = T),
        nr_stops_other = sum(stops_other_black, na.rm = T),
        nr_stops_traffic = sum(stops_traffic_black, na.rm = T),
        nr_stops_drug = sum(stops_drug_black, na.rm = T),
        nr_stops_loitering = sum(stops_loitering_black, na.rm = T)
    )

# Read in unit level variables and join with new outcome variables
unit_level_types_stop_race <-
    read_csv(file.path(dir, "unit_level.csv")) %>%
    full_join(types_of_stops_race) %>%
...
unit_level_list_type_race <-
    unit_level_types_stop_race %>%
    pivot_longer(
        cols = matches("^nr_stops_"),
        names_to = "group",
        values_to = "dep_var"
    ) %>%
    group_split(group)

names <- map_chr(unit_level_list_type_race, function(df) {unique(df[["group"]])})
names(unit_level_list_type_race) <- names

regression_groups_type_race <-
    map(unit_level_list_type_race,
        function(df) {
            femlm(dep_var ~
                      black_ratio +
                      ...
                      violent_cr_capita +
                      property_cr_capita +
                      nr_officer + offset(log(black)) | unit + year,
                  family = "negbin",
                  data = df)
        }
    )
