...
predict_results %>%
    filter(year == 2014,
           variable != "Property Crime Per 10,000",
           unit %in% c(6, 8, 19)) %>%
    mutate(type = case_when(type == "min" ~ "Min",
                            type == "max" ~ "Max",
                            type == "med" ~ "Med",
                            type == "p25" ~ "P25",
                            type == "p75" ~ "P75")) %>%
    mutate(type = factor(type, levels = c("Min", "P25", "Med", "P75", "Max"))) %>%
    ggplot(aes(x = type, y = predicted_black_stops)) +
    geom_point(aes(color = variable)) +
    geom_line(aes(color = variable, group = variable)) +
...
```{r}
predict_results_compare %>%
    filter(year == 2014, unit %in% c(6, 8, 19)) %>%
    mutate(type = case_when(type == "min" ~ "Min",
                            type == "max" ~ "Max",
                            type == "med" ~ "Med",
                            type == "p25" ~ "P25",
...
new_data_exp_prcnt <-
    ...
    group_by(unit, year, black) %>%
    summarise(prcnt_officer_FEMALE = mean(prcnt_officer_FEMALE),
              prcnt_officer_black = mean(prcnt_officer_black),
              violent_cr_capita = mean(violent_cr_capita),
              property_cr_capita = mean(property_cr_capita),
              log_total_officers = mean(log_total_officers),
              black = mean(black),
...
predict_results_exp_prcnt <-
    new_data_exp_prcnt %>%
    mutate(predicted_black_stops = predict(fixest_cluster_nb_prcnt_full,
                                           newdata = new_data_exp_prcnt) * black,
           type = factor(type, levels = c("min", "p25", "med", "p75", "max")),
           variable = "Years Worked In Unit (Mean)") %>%
    select(unit, year, type, variable, predicted_black_stops)

predict_results_total_prcnt <- rbind(predict_results_prcnt, predict_results_exp_prcnt)

predict_results_total_prcnt %>%
    filter(year == 2014, unit %in% c(6, 8, 19)) %>%
    mutate(type = case_when(type == "min" ~ "Min",
                            type == "max" ~ "Max",
                            type == "med" ~ "Med",
