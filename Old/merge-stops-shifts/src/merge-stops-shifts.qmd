...
officer_assignments_2 <- my_read_csv(here(in_path, "officer_assignments_risi.csv.gz"))
stops <-
    my_read_csv(here(in_path, "stops.csv")) %>%
    mutate(stop_officer_id = as.character(row_number()))
stops_2 <-
    my_read_csv(here(in_path, "stops.csv.gz")) %>%
    mutate(stop_officer_id = as.character(row_number()))

# convert to data.table
setDT(officer_assignments_2)
setDT(stops_2)
```
...
stops_merged <- ba_merge(officer_assignments_2, stops_2, stop_officer_id)
